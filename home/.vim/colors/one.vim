...
if has('gui_running') || &t_Co == 88 || &t_Co == 256
  ...
  if &background ==# 'dark'
    ...
    let s:hue_1_light = 'E0FFFF' " cyan light
    let s:hue_2_light = 'AFEEEE' " blue light
    let s:hue_3_light = 'E6E6FA' " purple tint
    let s:hue_4_light = '90EE90' " green tint
    let s:hue_5_light = 'FFE4E1' " red light
    let s:hue_6_light = 'FAF0E6' " orange light

    let s:syntax_bg     = 'ffffff'
    let s:syntax_gutter_bg = 'cccccc'
    let s:syntax_gutter = '444444'
  ...
  call <sid>X('LineNr',       s:syntax_gutter, s:syntax_gutter_bg, '')
  ...
  " Diff highlighting -------------------------------------------------------{{{
  call <sid>X('DiffAdd',     s:hue_4, s:hue_4_light, '')
  call <sid>X('DiffChange',  s:hue_6, s:hue_6_light, '')
  call <sid>X('DiffDelete',  s:hue_5, s:hue_5_light, '')
  call <sid>X('DiffText',    s:hue_2, s:hue_2_light, '')
  call <sid>X('DiffAdded',   s:hue_4, s:hue_4_light, '')
  call <sid>X('DiffFile',    s:hue_5, s:hue_5_light, '')
  call <sid>X('DiffNewFile', s:hue_4, s:hue_4_light, '')
  call <sid>X('DiffLine',    s:hue_2, s:hue_2_light, '')
  call <sid>X('DiffRemoved', s:hue_5, s:hue_5_light, '')
  " }}}
